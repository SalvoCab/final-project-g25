...
const EditContactModal: React.FC<EditContactModalProps> = ({contact, show, onHide, onContactEdited }) => {
        ...
        try {
            ...
            const preparedData = {
                ...
                emails: rawInput.emails
                    ...
                    .map((x) => x.trim())
                    .filter((x) => x),
                addresses: rawInput.addresses
                    .split(",")
                    .map((x) => x.trim())
                    .filter((x) => x),
                phoneNumbers: rawInput.phoneNumbers
                    .split(",")
                    .map((x) => x.trim())
                    .filter((x) => x),
            };
            const c = await updateContact(contact.id,preparedData );

            if (category === "professional") {
                await ensureCSRFToken();
                await createProfessional(c.id,professionalData);
            } else if (category === "customer") {
                await ensureCSRFToken();
                await createCustomer(c.id, customerNotes);
            }
        ...
        <Modal show={show} onHide={onHide} backdrop="static" centered size="lg">
            ...
            <Modal.Body>
                <Form>
                    ...
                    <Form.Group className="mb-3">
                        <Form.Label>Category{contact?.category !=="unknown" && <>: If you want to change the category you should <b>downgrade</b> it in <b>{contact?.category}s</b>!</>}</Form.Label>
                        {contact && (contact.category==="unknown"?
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control placeholder="email1@example.com, email2@example.com" value={rawInput.emails}  onChange={(e) =>
                            setRawInput((prev) => ({
                                ...prev,
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control placeholder="Via Roma 1, Via Verdi 2" value={rawInput.addresses}  onChange={(e) =>
                            setRawInput((prev) => ({
                                ...prev,
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control placeholder="123456, 7891011" value={rawInput.phoneNumbers}  onChange={(e) =>
                            setRawInput((prev) => ({
                                ...prev,
