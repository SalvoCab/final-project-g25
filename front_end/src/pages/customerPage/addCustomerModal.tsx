...
const AddCustomerModal: React.FC<AddCustomerModalProps> = ({ show, onHide, onCustomerAdded }) => {
    ...
    useEffect(() => {
        // Optionally fetch all contacts by default when modal opens
        if (show) handleFilter();
    }, [show]);
                        ...
                        >
                            ...
                            {contacts.map(contact => (
                                <option key={contact.id} value={contact.id}>
                                    {contact.name} {contact.surname} ({contact.emails.join(", ")}) ({contact.phoneNumber.join(", ")})
                                </option>
                            ))}
                        </Form.Select>
                    </Form.Group>
